...
class ModelManager {
  ...
  static const String _modelDownloadBaseUrl =
      'https://models.thinq.ai/files/yolo';
  ...
  Future<String?> _getIOSModelPath(ModelType modelType) async {
    ...
    try {
      ...
      if (bundleCheck['exists'] == true) {
        _updateStatus('Model found in bundle');
        return modelType.modelName;
      }
    } catch (e) {
      _updateStatus('Bundle check failed: $e');
    }
    ...
    if (await modelDir.exists()) {
      if (await File('${modelDir.path}/Manifest.json').exists()) {
        _updateStatus('Model found in local storage');
  ...
  Future<String?> _downloadIOSModel(ModelType modelType) async {
    ...
    try {
      _updateStatus('Checking for model in assets...');
      final zipData = await rootBundle.load(
        'assets/models/${modelType.modelName}.mlpackage.zip',
      );
      _updateStatus('Extracting model from assets...');
      ...
      _updateStatus('Asset not found, will download from internet: $e');
    }
  ...
  Future<String?> _getAndroidModelPath(ModelType modelType) async {
    ...
    if (bytes != null && bytes.isNotEmpty) {
      ...
      _updateStatus('Model saved to: ${modelFile.path}');
      return modelFile.path;
    }
    _updateStatus('Failed to download model from internet');
    return null;
  }
  ...
  Future<List<int>?> _downloadFile(String url) async {
    try {
      ...
      if (request.statusCode != 200) {
        _updateStatus('Download failed: HTTP ${request.statusCode}');
        client.close();
        return null;
      }
      ...
      client.close();
      _updateStatus('Download complete (${bytes.length} bytes)');
      return bytes;
    } catch (e) {
